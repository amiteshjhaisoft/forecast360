...
def sidebar_getting_started():
    ...
    with st.sidebar:
        ...
        if up is not None:
            try:
                ...
                if _data is not None and hasattr(_data, "shape"):
                    ...
                    # Keep the original upload separately. Do not reuse the cleaned dataframe
                    # for re-detection because a wrong date selection can clean it down to 0 rows.
                    st.session_state["raw_uploaded_df"] = _data.copy()
        ...
        def _to_datetime_safe(s: pd.Series) -> pd.Series:
            """Parse dates safely across pandas versions and mixed upload formats."""
            # pandas >= 2 supports format="mixed" and is useful for uploaded CSV/Excel files.
            # Older pandas versions may not support it, so fall back gracefully.
            try:
                parsed = pd.to_datetime(s, errors="coerce", format="mixed")
            except TypeError:
                parsed = pd.to_datetime(s, errors="coerce")

            # If dd/mm/yyyy is more likely than mm/dd/yyyy, try dayfirst as a fallback.
            if parsed.notna().mean() < 0.80:
                try:
                    parsed_dayfirst = pd.to_datetime(s, errors="coerce", format="mixed", dayfirst=True)
                except TypeError:
        ...
        def _infer_time_col_fallback(df: pd.DataFrame) -> str | None:
            ...
            # 1) Already typed datetime columns are the safest.
            for c in df.columns:
                if pd.api.types.is_datetime64_any_dtype(df[c]):
                    return c

            # 2) Prefer column names that normally indicate dates.
            name_hints = ("date", "time", "timestamp", "period", "month", "year", "week", "invoice")
            hinted_cols = [c for c in df.columns if any(h in str(c).lower() for h in name_hints)]
            other_cols = [c for c in df.columns if c not in hinted_cols]

            best_col, best_score = None, 0.0
            for c in hinted_cols + other_cols:
                s = df[c]
                if pd.api.types.is_numeric_dtype(s):
                    continue
                parsed = _to_datetime_safe(s)
                score = float(parsed.notna().mean()) if len(parsed) else 0.0
                # Hinted columns can pass with a slightly lower threshold; non-hinted columns
                # must be very date-like so text columns such as Retailer are not selected.
                threshold = 0.60 if c in hinted_cols else 0.85
        ...
        if _data is None:
            _data = st.session_state.get("raw_uploaded_df")
            if _data is None:
        ...
        try:
            if "infer_time_col" in globals() and callable(globals().get("infer_time_col")):
                guess_date = infer_time_col(_data)
        except Exception:
            guess_date = None

        # Always fall back to the local robust detector if the global detector fails
        # or returns an invalid/non-date column. This prevents columns like Retailer
        # from being selected as the Date/Time column.
        if guess_date is None or guess_date not in _data.columns:
            guess_date = _infer_time_col_fallback(_data)

        if guess_date is None or guess_date not in _data.columns:
            st.warning("Could not auto-detect a valid Date/Time column. Please select the correct date column manually.")
            guess_date = _data.columns[0]

        guess_target = _infer_target(_data)

        # Reset a stale/invalid date selection from a previous run or previous upload.
        # Without this, Streamlit can keep "Retailer" selected even after we detect
        # "Invoice Date" correctly because the widget key value is stored in session_state.
        current_date_col = st.session_state.get("date_col")
        if guess_date in _data.columns:
            if current_date_col not in _data.columns:
                st.session_state["date_col"] = guess_date
            else:
                current_ratio = float(_to_datetime_safe(_data[current_date_col]).notna().mean()) if len(_data) else 0.0
                guess_ratio = float(_to_datetime_safe(_data[guess_date]).notna().mean()) if len(_data) else 0.0
                if current_ratio < 0.50 and guess_ratio >= current_ratio:
                    st.session_state["date_col"] = guess_date

        date_col = st.selectbox(
            "Date/Time column (auto-detected)",
            list(_data.columns),
            index=list(_data.columns).index(st.session_state.get("date_col", guess_date)) if st.session_state.get("date_col", guess_date) in _data.columns else 0,
            key="date_col",
        ...
        if st.session_state.get("target_col") not in target_options:
            st.session_state["target_col"] = guess_target if guess_target in target_options else target_options[0]
        target_index = target_options.index(st.session_state["target_col"])
        ...
        parsed_dates = _to_datetime_safe(_data[date_col])
        valid_date_ratio = float(parsed_dates.notna().mean()) if len(parsed_dates) else 0.0
        if parsed_dates.notna().sum() == 0 or valid_date_ratio < 0.50:
            st.session_state["uploaded_df"] = pd.DataFrame()
            st.error(
                f"Selected Date/Time column '{date_col}' does not contain enough valid dates. "
                "Please choose the real date column, for example 'Invoice Date', 'Order Date', or similar."
            )
            return

        _data[date_col] = parsed_dates
