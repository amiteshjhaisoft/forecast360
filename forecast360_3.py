...
# --- Optional pandas import (module works even if pandas is missing) ---
try:
    import pandas as pd  # only for data handling / type hints
    _HAS_PD = True
except Exception:
    pd = None  # type: ignore[assignment]
    _HAS_PD = False

# --- External snapshot writer ---
# Expects: capture_forecast360_results(page_name, models_summary_text, best_model, metrics,
#                                      input_table, output_table, run_tag, name, min_interval_sec)
from f360_llm_snapshot import capture_forecast360_results


# ---------- Utilities ----------
def _is_dataframe(obj: Any) -> bool:
    """True iff pandas is available and obj is a DataFrame."""
    return _HAS_PD and isinstance(obj, pd.DataFrame)  # type: ignore[arg-type]


# ---------- Arrow/Parquet sanitiser ----------
def _sanitize_for_arrow(df: Any) -> Any:
    """
    ...
    Rules:
      ...
      - mixed-type object columns:
          ...
          * otherwise       -> "string[pyarrow]"
      - native numeric/bool -> Arrow-friendly dtypes (nullable where useful)

    If pandas is unavailable or input isn't a DataFrame, return input unchanged.
    """
    if not _is_dataframe(df) or df.empty:  # type: ignore[attr-defined]
        return df
    ...
    for c in out.columns:
        ...
        try:
            import numpy as _np
            non_na = s.dropna()
            if len(non_na) > 0 and non_na.map(type).isin({bool, _np.bool_}).all():
        ...
        try:
            if s.dtype == "object":
                num_try = pd.to_numeric(s, errors="coerce")  # type: ignore[union-attr]
                if num_try.notna().mean() >= 0.90:
                    out[c] = num_try
                else:
        ...
        # Native dtypes: normalize to Arrow-friendly
        try:
            if pd.api.types.is_bool_dtype(s):  # type: ignore[union-attr]
                out[c] = s.astype("boolean[pyarrow]")
            elif pd.api.types.is_integer_dtype(s):  # type: ignore[union-attr]
                out[c] = s.astype("Int64")
            elif pd.api.types.is_float_dtype(s):  # type: ignore[union-attr]
...
def st_df(obj: Any, *args, **kwargs):
    """Streamlit dataframe with automatic Arrow-safe sanitization (if pandas is present)."""
    try:
        if _HAS_PD:
            ...
            if isinstance(obj, pd.DataFrame):  # type: ignore[arg-type]
...
# ---------- Tiny summary synthesis ----------
def _fallback_summary(
    best_model: Optional[str],
    leaderboard: Optional[Union[Dict[str, Any], List[Dict[str, Any]]]],
) -> Optional[str]:
            ...
            for r in leaderboard:
                if isinstance(r, dict):
                    ...
                    metrics_str = ", ".join(
                        f"{k.upper()}={v}"
                        for k, v in r.items()
                        if isinstance(k, str) and k.lower() in ("rmse", "mape", "mae", "smape", "mase")
                    )
                    if model and metrics_str:
                        lines.append(f"- {model}: {metrics_str}")
                    elif model:
                        lines.append(f"- {model}")
            if lines:
                parts.append("Leaderboard:\n" + "\n".join(lines))
        elif isinstance(leaderboard, dict):
            items = [f"- {k}: {v}" for k, v in leaderboard.items()]
            if items:
...
def gather_and_snapshot_forecast360(
    ...
    *,
    ...
    Looks for common keys:
      - input table:   'dfd' | 'main_df'
      - output table:  'forecast_df' | 'yhat_df' | 'output_df'
      - leaderboard:   rows  -> 'leaderboard_rows' | 'leaderboard_list'
                       dict  -> 'leaderboard_metrics_dict' | 'headline_metrics'
      - best model:    'best_model_name' | 'best_model'
      - summary text:  'summary_text' (else synthesized)
    """
    ss = st.session_state
    ...
    # Ensure Arrow-safe tables
    if _is_dataframe(main_df):
        main_df = _sanitize_for_arrow(main_df)
    if _is_dataframe(forecast_df):
        forecast_df = _sanitize_for_arrow(forecast_df)

    # Leaderboard / metrics / best model
    ...
    if isinstance(leaderboard_rows, list) and leaderboard_rows:
        metrics_payload["leaderboard"] = [r for r in leaderboard_rows if isinstance(r, dict)]
    ...
    try:
        capture_forecast360_results(
            ...
            input_table=(main_df if _is_dataframe(main_df) else None),
            output_table=(forecast_df if _is_dataframe(forecast_df) else None),
            run_tag=run_tag,
            name=snapshot_name,
            min_interval_sec=min_interval_sec,
        )
        st.toast("One-pager snapshot saved for Decision Intelligence.", icon="✅")
    except Exception as e:
        st.error(f"Snapshot failed: {e}")


def snapshot_now() -> None:
    """Convenience alias with defaults for quick snapshots."""
    gather_and_snapshot_forecast360()
    ...
    try:
        ...
        if callable(get_kb):
            ...
            if kb is not None:
                ...
                tags = ["auto", "snapshot"]
                ingest_enabled = bool(st.session_state.get("kb_auto_ingest", False))
                last_ts = float(st.session_state.get("_last_kb_ingest_ts", 0.0))
                now_ts = time.time()
                if ingest_enabled and (now_ts - last_ts) >= 45:
                    st.session_state["_last_kb_ingest_ts"] = now_ts
...
def _build_kb_context_from_snapshot(snap: dict, top_k: int = 6, max_chars: int = 4000) -> str:
    ...
    def _add(key: str, payload):
        ...
        try:
            if isinstance(payload, tuple) and len(payload) == 2:
                kind, val = payload
                if kind == "df_json":
                    if isinstance(val, str) and len(val) > 120_000:
                        try:
                            j = _json.loads(val)
                            r, c = len(j.get("data", [])), len(j.get("columns", []))
                            parts.append(f"[{key}] DataFrame shape=({r}, {c}) [preview skipped]")
                        except Exception:
                            parts.append(f"[{key}] [large table] preview skipped")
                    else:
                    ...
                    if isinstance(val, (dict, list)):
                        parts.append(f"[{key}] {_json.dumps(val, ensure_ascii=False)[:1000]}")
                    else:
    ...
    text = "\n\n".join(parts)
    return (text[:max_chars] + "\n...[truncated]") if len(text) > max_chars else (text or "Snapshot exists but contains no readable artifacts.")
...
# --------------------------- default naive ---------------------
        ...
        X_full = X.reindex(y_tr.index) if not X.empty else pd.DataFrame(index=y_tr.index)
    ...
    if st.session_state["di_modal_open"]:
        if _di_modal is not None:
            ...
            try:
                ...
                st.switch_page("pages/decision_intelligence.py")
            except Exception:
                st.warning(
                    "This build lacks `st.dialog`, and a DI page isn't available. "
                    "Create 'pages/decision_intelligence.py' and call run_di_chat()."
                )
...
def render_kb_auto_ingest_toggle():
    try:
        st.toggle("🔁 Auto-ingest snapshots to Knowledge Base", key="kb_auto_ingest", value=False,
                  help="Turn ON only when needed. Debounced to reduce I/O.")
    except Exception:
        pass


def show_df_fast(df, name: str = 'table', max_rows: int = 2000):
    try:
        import pandas as _pd
        if not isinstance(df, _pd.DataFrame) or df.empty:
            st.info(f"{name}: (empty)")
            return
        n = len(df)
        view = df if n <= max_rows else df.head(max_rows)
        st.caption(f"{name}: showing {len(view):,} of {n:,} rows")
        try:
            st_df(view, use_container_width=True)
        except Exception:
            st.dataframe(view, use_container_width=True)
        csv = df.to_csv(index=False).encode('utf-8')
        st.download_button(f"⬇️ Download full {name} as CSV",
                           data=csv, file_name=f"{name}.csv", mime='text/csv')
    except Exception as _e:
        st.warning(f"{name}: preview unavailable ({_e})")


@st.cache_data(show_spinner=True, ttl=3600, max_entries=16)
def _cached_perm_importance(predict_fn, y_true, exog_df, window=200, metric='rmse', rs=42):
    return _permute_ts_importance(predict_fn, y_true, exog_df, window=window, metric=metric, random_state=rs)
...
def timeblock(label: str):
    ...
    logs = st.session_state.setdefault('_perf_logs', [])
    logs.append(f"{label}: {dt:.1f} ms")

def show_perf():
    logs = st.session_state.get('_perf_logs', [])
    if logs:
        with st.expander('⏱ Performance (this run)'):
            for line in logs:
                st.write(line)
